...
DB_FILE = os.path.join(os.path.dirname(os.path.abspath(__file__)), "ucp_database_v1.db")
...
def init_db():
    ...
    # Create Products Table (Clean V1: only inventory attributes)
    cursor.execute("""
    ...
    CREATE TABLE IF NOT EXISTS purchases (
        ...
        status TEXT NOT NULL, -- PENDING, APPROVED, REJECTED, PROCESSED_PIX
    ...
    if cursor.fetchone()["count"] == 0:
        ...
        conn.commit()
        print("Database V1 initialized and seeded successfully.")
    else:
        print("Database V1 already initialized.")
...
def mark_batch_sent(batch_id):
    conn = get_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT sent_to_bank FROM pix_batches WHERE batch_id = ?", (batch_id,))
    row = cursor.fetchone()
    if not row or row["sent_to_bank"] == 1:
        conn.close()
        return False
