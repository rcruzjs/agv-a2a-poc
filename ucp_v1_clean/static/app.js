// Real-Time Dashboard Javascript Logic (V1 Clean Optimized)
...
// 1. Surgical DOM Updates for Inventory Grid
function updateStockGrid(products) {
    ...
    // If container currently holds the loading placeholder, clear it
    if (productsContainer.querySelector("[style*='text-align: center']")) {
    ...
    products.forEach(p => {
        ...
        let card = document.getElementById(`card-${p.sku}`);

        if (!card) {
            // Create brand new card structure on first load
            ...
            card.innerHTML = `
                <div class="product-meta" style="display: flex; align-items: center; gap: 15px; width: 100%; margin-bottom: 0.6rem;">
                    ...
                    <div class="product-qty-info" style="font-weight: 500; font-size: 0.95rem; white-space: nowrap;">
                        <span class="qty-text"></span> / ${p.qtd_minima} <span style="font-size: 0.75rem; color: var(--text-secondary); font-weight: normal;">(Mín)</span>
                    </div>
                </div>
        ...
        // Perform surgical micro-updates on properties instead of re-building HTML
        const qtyTextSpan = card.querySelector(".qty-text");
        const qtyInfoDiv = card.querySelector(".product-qty-info");
        const progressBar = card.querySelector(".progress-bar");

        // 1. Update stock count text
        qtyTextSpan.innerText = p.qtd_atual;

        // 2. Toggle quantity text colors dynamically
        qtyInfoDiv.style.color = isBreached ? 'var(--color-rose)' : 'var(--color-emerald)';

        // 3. Update progressive width & alert classes
        progressBar.style.width = `${progressPercentage}%`;
        if (isBreached) {
            progressBar.classList.add("status-breach");
            progressBar.classList.remove("status-ok");
        } else {
            progressBar.classList.add("status-ok");
            progressBar.classList.remove("status-breach");
        }
    });
}

// 2. High-Performance Buffered Console Logs
function addConsoleLog(message) {
    // 1. Accumulate log in the memory buffer
    logHistoryBuffer.push(message);
    if (logHistoryBuffer.length > MAX_LOGS) {
        logHistoryBuffer.shift(); // keep buffer capped
    }

    // 2. Render only if active view is Cockpit
    if (activeTab === "stock") {
...
function renderLogToDOM(message) {
    ...
    // Dynamic coloring based on event content
    if (message.includes("📥")) logLine.style.color = "#a5b4fc";
    else if (message.includes("⚠️")) logLine.style.color = "#f43f5e";
    else if (message.includes("🤖")) logLine.style.color = "#c084fc";
    else if (message.includes("🔍") || message.includes("⚖️")) logLine.style.color = "#38bdf8";
    else if (message.includes("✅")) logLine.style.color = "#34d399";
    else if (message.includes("❌")) logLine.style.color = "#fb7185";

    logLine.innerText = message;
    consoleRoot.appendChild(logLine);

    // Dom capping limit enforcement
    if (consoleRoot.children.length > MAX_LOGS) {
...
// Rebuilds entire console view instantly from memory when returning to Cockpit tab
function rebuildConsoleFromBuffer() {
    consoleRoot.innerHTML = "";
    logHistoryBuffer.forEach(msg => renderLogToDOM(msg));
}

// 3. Human-in-the-Loop A2UI Action Card
...
function handlePurchaseAction(orderId, action) {
    ...
    .then(response => {
        if (!response.ok) {
            throw new Error(`Erro ao enviar decisão: ${response.statusText}`);
        }
...
// 4. Dynamic Tab Navigation & Lazy Processing Activation
...
function switchTab(tabName) {
    ...
    if (tabName === "stock") {
        ...
        // Re-hydrate logs DOM from memory buffer instantly when returning
        rebuildConsoleFromBuffer();
    } else {
...
// 5. Pix Batching Queue Operations
